...
const getRecentOrders = async (match, limit = 5) => {
  const orders = await Order.find(match)
    .sort({ createdAt: -1 })
    .limit(limit)
    .select("orderNumber buyerId buyerType items totalAmount status createdAt")
    .lean();
...
const getCompletedOrderSales = async () => {
  ...
  return result.length ? toNumber(result[0].total) : 0;
};

const getSellerSales = async (sellerType, sellerId) => {
  const result = await Order.aggregate([
    {
      $unwind: "$items"
    },
...
const getBuyerProcurement = async (buyerId) => {
  ...
  return {
    ...
    totalAmount: toNumber(result[0].totalAmount)
  };
};
...
const getInventoryStats = async (productIds) => {
  ...
  return {
    ...
    availableStock: toNumber(result[0].availableStock)
  };
};
...
const getFarmerDashboard = async (user) => {
  ...
  const products = await Product.find(productMatch).select("_id").lean();
  ...
  const [
    ...
    getSellerSales("FARMER", farmerProfile._id),
  ...
  return {
    ...
    dashboard: {
      summary: {
        ...
        completedOrders: orderStatusCounts.delivered,
  ...
  return {
    ...
    dashboard: {
      summary: {
        ...
        totalProcurement: procurement.totalQuantity,

        totalSpending: procurement.totalAmount
      },
...
const getFpoDashboard = async (user) => {
  ...
  if (!fpoProfile) {
    throw createServiceError("FPO profile not found", 404);
  }
  ...
  const [
    ...
    getSellerSales("FPO", fpoProfile._id),
  ...
  return {
    ...
    dashboard: {
      summary: {
        ...
        totalOrders: getTotalFromStatusCounts(orderStatusCounts),
...
const getGovernmentDashboard = async (user) => {
  ensureRole(user, ROLES.GOVERNMENT_OFFICER);
...
const getMyDashboard = async (user) => {
  ...
  switch (role) {
    ...
    default:
      throw createServiceError("Unsupported dashboard role", 403);
  }
};
