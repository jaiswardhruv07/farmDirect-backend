...
const adminCreateProduct = async (user, data) => {
  ...
  const sellerProfile = await getSellerProfileById(sellerType, sellerId);
...
const adminUpdateProduct = async (user, productId, data) => {
  ...
  if (data.sellerType || data.sellerId) {
    const sellerType = data.sellerType || product.sellerType;

    const sellerId = data.sellerId || product.sellerId;
...
const approveProduct = async (user, productId) => {
  ...
  product.approval.status = PRODUCT_APPROVAL_STATUS.APPROVED;

  product.approval.reviewedBy = user.id || user._id;
...
const rejectProduct = async (user, productId, rejectionReason) => {
  ...
  product.approval.status = PRODUCT_APPROVAL_STATUS.REJECTED;

  product.approval.reviewedBy = user.id || user._id;

  product.approval.reviewedAt = new Date();

  product.approval.rejectionReason = rejectionReason;
