...
const orderAddressSchema = Joi.object({
  ...
  addressLine2: Joi.string().trim().max(200).allow("", null).default(null),

  village: Joi.string().trim().max(100).allow("", null).default(null),
...
const createOrderSchema = Joi.object({
  items: Joi.array().items(orderItemCreateSchema).min(1).required(),
